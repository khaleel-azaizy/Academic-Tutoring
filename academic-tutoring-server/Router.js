...
app.get('/api/teacher/lessons/:lessonId/resources', authenticateToken, authorizeRole('Teacher'), async (req, res) => {
  try {
    ...
    const resources = await db.collection('lesson_resources')
      .find({ lessonId: lesson._id, deletedAt: { $exists: false } })
      .sort({ createdAt: -1 })
      .toArray();
    return res.json({ resources });
  } catch (error) {
    console.error('Get lesson resources error:', error);
    return res.status(500).json({ error: 'Failed to load resource links' });
  }
});
...
app.delete('/api/teacher/lessons/:lessonId/resources/:resourceId', authenticateToken, authorizeRole('Teacher'), async (req, res) => {
  try {
    ...
    if (!ObjectId.isValid(lessonId) || !ObjectId.isValid(resourceId)) {
      return res.status(400).json({ error: 'Invalid lessonId or resourceId' });
    }
    const lesson = await db.collection('lessons').findOne({
      _id: new ObjectId(lessonId),
      teacherId: new ObjectId(req.user._id)
    ...
    const result = await db.collection('lesson_resources').updateOne(
      { _id: new ObjectId(resourceId), lessonId: lesson._id, deletedAt: { $exists: false } },
      { $set: { deletedAt: new Date() } }
    );
    if (result.matchedCount === 0) {
      return res.status(404).json({ error: 'Resource not found' });
    }
    return res.json({ message: 'Resource link removed' });
  } catch (error) {
    console.error('Delete lesson resource error:', error);
    return res.status(500).json({ error: 'Failed to delete resource link' });
  }
});
...
app.get('/api/student/lessons/:lessonId/resources', authenticateToken, authorizeRole('Student'), async (req, res) => {
  try {
    ...
    console.error('Student lesson resources error:', error);
    return res.status(500).json({ error: 'Failed to load resource links' });
  }
});
